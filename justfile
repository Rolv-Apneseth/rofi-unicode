...
alias tb := test-bare
alias d := develop
alias uu := update-unicode

# VARIABLES ----------------------------------------------------------------------------------------

PKGNAME := env("PKGNAME", "rofi-unicode")
PKGDIR := env("PKGDIR", "")
LIB_NAME := "librofi_unicode.so"
...
# Update Unicode code points
update-unicode:
    cargo run --release --manifest-path ./gen-unicode/Cargo.toml
...
test $THEME:
    rofi -modi unicode -show unicode -theme $THEME
